...
// helper that returns either the base URL or the app instance
async function getRequestTarget() {
  if (process.env.API_BASE_URL) {
    return BASE_URL;
  }
  // dynamic import to avoid ESM parsing errors in CommonJS tests
...
async function createAndLoginTestUser() {
  ...
  const target = await getRequestTarget();
  ...
  // VERIFY
  // The API now requires a username when completing verification, so generate one.
  const username = `user_${Date.now()}`;
  const verifyRes = await request(target)
  ...
  if (![200, 201].includes(verifyRes.status)) {
    throw new Error(
      `User verification failed: ${verifyRes.status} ${verifyRes.text}`
    );
  }

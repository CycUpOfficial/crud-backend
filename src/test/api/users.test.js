...
describe("Users API", () => {
    ...
    describe("Authenticated", () => {
        ...
        test("PUT /users/profile -> updates profile and returns 200", async () => {
            const payload = await buildProfilePayload({ firstName: "Joan" });
    ...
    describe("Unauthenticated", () => {
        test("GET /users/profile -> 401", async () => {
        ...
        test("PUT /users/profile -> 401", async () => {
            const res = await request(app)
                .put(`${API_PREFIX}/users/profile`)
                .send({ firstName: "John" });
        ...
        test("PUT /users/profile invalid payload -> 400 or 401", async () => {
            const res = await request(app)
                .put(`${API_PREFIX}/users/profile`)
